...
void Tutorial21_RayTracing::RecreateDynamicResources()
{
        m_pDevice->IdleGPU();
    ...
    m_pTLAS.Release();
    m_pProceduralBLAS.Release();
    m_BoxAttribsCB.Release();                 m_SphereMaterialsBufferSRV.Release();     m_SphereMaterialsBuffer.Release();    
        m_SphereMaterials.resize(m_NumSpheres);
    std::mt19937                          gen(12345);     std::uniform_real_distribution<float> dist(0.0f, 1.0f);
    ...
    for (Uint32 i = 0; i < m_NumSpheres; ++i)
    {
        ...
        if (i % 3 == 0) m_SphereMaterials[i].IndexOfRefraction = 1.01f + 0.1f * dist(gen);
        else if (i % 3 == 1)
            m_SphereMaterials[i].IndexOfRefraction = 1.4f + 0.3f * dist(gen);
        else
            m_SphereMaterials[i].IndexOfRefraction = 1.7f + 0.3f * dist(gen);
        m_SphereMaterials[i].Radius = (i < 2) ? LargeSphereRadius : SmallSphereRadius;
    }
    ...
    SphereMatBuffDesc.Usage             = USAGE_IMMUTABLE;
    ...
    BufferData SphereMatData            = {m_SphereMaterials.data(), SphereMatBuffDesc.Size};
    ...
    if (m_pRayTracingPSO)
    {
        if (auto* pVar = m_pRayTracingPSO->GetStaticVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "g_SphereMaterials"))
            pVar->Set(m_SphereMaterialsBufferSRV);
    }


            CreateProceduralBLAS();     UpdateTLAS();           
    CreateSBT(); }
...
void Tutorial21_RayTracing::CreateProceduralBLAS()
{
...

    {
        ...
        {
            BoxInfo.GeometryName = "ProceduralSpheres";                         BoxInfo.MaxBoxCount = m_NumSpheres;

            BottomLevelASDesc ASDesc;
            ASDesc.Name     = "Procedural BLAS";
            ASDesc.Flags    = RAYTRACING_BUILD_AS_PREFER_FAST_TRACE;             ASDesc.pBoxes   = &BoxInfo;                                          ASDesc.BoxCount = 1;                                     
        ...
        BoxData.GeometryName = BoxInfo.GeometryName;                 BoxData.BoxCount = m_NumSpheres;
                BoxData.BoxStride = sizeof(HLSL::BoxAttribs);
                BoxData.pBoxBuffer = m_BoxAttribsCB;
        BoxData.BoxOffset  = 0; 

                BuildBLASAttribs Attribs;
        Attribs.pBLAS        = m_pProceduralBLAS;         Attribs.pBoxData     = &BoxData;                  Attribs.BoxDataCount = 1;                 
        Attribs.pScratchBuffer = pScratchBuffer; 
                Attribs.BLASTransitionMode          = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
        Attribs.GeometryTransitionMode      = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
        Attribs.ScratchBufferTransitionMode = RESOURCE_STATE_TRANSITION_MODE_TRANSITION;
...
{
    const Uint32 TotalInstances = 1 + m_NumSpheres;
            ...
            if (!m_pTLAS)
    {
        ...
        VERIFY_EXPR(m_pTLAS != nullptr);         NeedUpdate = false;
        if (m_pRayTracingSRB)
        {
        ...
        if (m_NumSpheres > 2)
    {
        ...
        for (Uint32 i = 2; i < m_NumSpheres; ++i)
        {
            ...
            AnimateOpaqueSphere(Instances[i]);
        }
    }
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    BufferDesc CBDesc;     CBDesc.Name      = "Constant buffer";
    CBDesc.Size      = sizeof(m_Constants);     CBDesc.Usage     = USAGE_DEFAULT;           CBDesc.BindFlags = BIND_UNIFORM_BUFFER;
    ...
    VERIFY_EXPR(m_ConstantsCB != nullptr);
        ...
        m_SphereMaterials.resize(m_NumSpheres);
    std::mt19937                          gen(12345);     std::uniform_real_distribution<float> dist(0.0f, 1.0f);
    std::uniform_real_distribution<float> ior_dist(1.01f, 2.0f);        const float                           LargeSphereRadius = 3.0f;     const float                           SmallSphereRadius = 0.5f;     for (Uint32 i = 0; i < m_NumSpheres; ++i)
    {
                m_SphereMaterials[i].BaseColor = float4(dist(gen), dist(gen), dist(gen), 1.0f);
        m_SphereMaterials[i].Roughness = 0.05f + 0.9f * dist(gen);
        m_SphereMaterials[i].Metallic  = (dist(gen) > 0.5f) ? (0.7f + 0.3f * dist(gen)) : (0.0f + 0.2f * dist(gen));
                ...
                if (i % 3 == 0)
            m_SphereMaterials[i].IndexOfRefraction = 1.01f + 0.1f * dist(gen);         else if (i % 3 == 1)
            m_SphereMaterials[i].IndexOfRefraction = 1.4f + 0.3f * dist(gen);         else
            m_SphereMaterials[i].IndexOfRefraction = 1.7f + 0.3f * dist(gen); 
        if (i < 2)         {
    ...
    m_SphereMaterialsBufferSRV = m_SphereMaterialsBuffer->GetDefaultView(BUFFER_VIEW_SHADER_RESOURCE);
        ...
        CreateGraphicsPSO();
    CreateRayTracingPSO();     LoadTextures();
    CreateCubeBLAS();           CreateProceduralBLAS();     UpdateTLAS();               CreateSBT();            
